SET ECHO ON
SPOOL "C:\Users\ASUS\OneDrive\Documents\SAIT\Courses\CPRG-250-SA1\Week 7\Project_Deliverable\TimberScript.txt"
...
CREATE TABLE TB_Customer_Order(
    ...
    amt_tax AS (SELECT tax_rate FROM TB_tax JOIN TB_Customer_Order ON ship_prov = tax_prov WHERE ship_prov = tax_prov),
    ...
    CONSTRAINT SYS_TB_Customer_Order_shipProv_UK UNIQUE (ship_prov)
);

CREATE TABLE TB_Tax(
    tax_prov CHAR(2)CONSTRAINT SYS_TB_TAX_PK PRIMARY KEY, 
    tax_rate NUMBER(4,3) NOT NULL,
    CONSTRAINT SYS_TB_Tax_tax_prov_CK CHECK(tax_prov in ('BC', 'AB', 'SK', 'MB', 'ON', 'QC', 'NB', 'NS', 'NL', 'NT', 'NU', 'PE','YT') ),
    CONSTRAINT SYS_TB_CusOrd_Tax_FK FOREIGN KEY (tax_prov) REFERENCES TB_Customer_Order (ship_prov)
);
...
CREATE TABLE TB_Product_Review(
    ...
    rating number(1) CONSTRAINT SYS_TB_Product_Review_Rating_CK CHECK (rating between 1 and 5),
